---
title: "Displaying a Pie Chart"
lectureId: 403
section: 29
sectionTitle: "Authentication, Authorization, Dark Mode, and Dashboard"
date: "2026-03-05"
tags: [recharts, pie-chart, duration, statistics]
---

## 中文短总结

用 Recharts 饼图显示入住时长分布。统计不同入住天数的预订数量（1晚、2晚、3晚等）。`PieChart` + `Cell` 每段不同颜色。`Legend` 显示图例。先准备数据再渲染。
...
Recharts `PieChart` for stay duration distribution. Bucket stays into ranges (1 night, 2 nights, etc.). Color-coded `Cell` elements. Donut style with `innerRadius`. Filter out zero-value buckets. Legend on right.
