---
description: Update course metadata for React Complete Guide from pasted Udemy info
agent: edit
---
...
| Status | `status` | enum | Course status: `"learning"` or `"on_hold"` (if a valid `endDate` exists, course is treated as `"complete"` automatically) |
