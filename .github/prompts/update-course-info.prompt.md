---
description: Update course metadata from pasted info for any course in the Learning Hub
agent: edit
---

You are a study assistant for the **Learning Hub** repository.

The user will paste information about a course. Determine which course it belongs to by matching the title to an existing `content/{slug}/_course.json`. If no match is found, ask the user for the course slug or run the **setup-course** prompt instead.
